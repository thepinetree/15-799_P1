...
class QueryParser():
	...
	def parse(self, query: str) -> QueryAttributes:
		...
		if (num_queries != 1):
			print("Queries must be parsed independently. Got %d, expected 1.", num_queries)
			assert(False)
...
if __name__ == "__main__":
	qp = QueryParser()
	res = qp.parse('''SELECT * FROM review r, item i WHERE i.i_id = r.i_id and r.i_id=112 ORDER BY rating DESC, r.creation_date DESC LIMIT 10''')
	print(res)
